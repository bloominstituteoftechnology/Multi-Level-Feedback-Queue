...
class Process {
    constructor(pid, cpuTimeNeeded = null, blocking = false) {
        this._pid = pid;
        this.queue = null;
        this.cpuTimeNeeded = (cpuTimeNeeded !== null)
            ? cpuTimeNeeded
            : Math.round(Math.random() * 1000);
        this.blockingTimeNeeded = blocking
            ? Math.round(Math.random() * 100)
            : 0;
        // A bool representing whether this process was toggled from blocking to
        // non-blocking or vice versa
    ...
    // If no blocking time is needed by this process, decrement the amount of CPU
    // time it needs by the input time If blocking time is needed by this process,
    // move it to the blocking queue by emitting the appropriate interrupt Make sure
    ...
    // If this process requires blocking time, decrement the amount of blocking time
    // it needs by the input time Once it no longer needs to perform any blocking
    ...
    executeBlockingProcess(time) {
        ...
        if (this.blockingTimeNeeded === 0) {
            this
                .queue
                .emitInterrupt(this, SchedulerInterrupt.PROCESS_READY);
            this.stateChanged = true;
        }
    }
