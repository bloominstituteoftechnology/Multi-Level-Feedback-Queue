...
class Process {
    constructor(pid, cpuTimeNeeded = null, blocking = false) {
        this._pid = pid;
        this.queue = null;
        this.cpuTimeNeeded = (cpuTimeNeeded !== null) ? cpuTimeNeeded : Math.round(Math.random() * 1000); // not a real thing...
        this.blockingTimeNeeded = blocking ? Math.round(Math.random() * 100) : 0; // also not real
        // A bool representing whether this process was toggled from blocking to non-blocking or vice versa
        this.stateChanged = false; // if process has jumped between the running state and a blocking state
    }
