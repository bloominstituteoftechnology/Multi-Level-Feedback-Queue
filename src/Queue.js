...
class Queue {
  ...
  manageTimeSlice(currentProcess, time) {
    if (currentProcess.isStateChanged()) {
      ...
      if (this.quantumClock >= this.quantum) {
        this.quantumClock = 0;
        const dqprocess = currentProcess.queue.dequeue();
  ...
  doBlockingWork(time) {
    const currentProc = this.peek();
    currentProc.executeBlockingProcess(time);
    this.manageTimeSlice(currentProc, time);
  }
  ...
  emitInterrupt(source, interrupt) {
    ...
    case 'PROCESS_BLOCKED':
      this.scheduler.emitInterrupt(this, foundProcess, SchedulerInterrupt.PROCESS_BLOCKED);
      break;
    case 'PROCESS_READY':
      this.scheduler.emitInterrupt(this, foundProcess, SchedulerInterrupt.PROCESS_READY);
      break;
    }
  }
}
