...
class Queue {
  constructor(scheduler, quantum, priorityLevel, queueType) {
    ...
    // The queue's allotted time slice; each process in this queue is executed for this amount of
    // time
  ...
  // The queue's interrupt handler for notifying when a process needs to be moved to a different
  // queue

  // Receives a source process and an interrupt string

  // Find the index of the source process in `this.processes` and splice the process out of the
  // array

  // In the case of a PROCESS_BLOCKED interrupt, emit the appropriate scheduler interrupt to the
  // scheduler's interrupt handler

  // In the case of a PROCESS_READY interrupt, emit the appropriate scheduler interrupt to the
  // scheduler's interrupt handler
  emitInterrupt(source, interrupt) {
    const srcIndex = this.processes.indexOf(source);
    this.processes.splice(srcIndex, 1);
