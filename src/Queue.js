...
class Queue {
    ...
    manageTimeSlice(currentProcess, time) {
      ...
      if (this.quantumClock > this.quantum) {
        this.quantumClock = 0;
        const process = this.dequeue(); //maybe is finished of this??
        if (!process.isFinished()) this.scheduler.handleInterrupt(this, process, SchedulerInterrupt.LOWER_PRIORITY);
      }
    }
    ...
    doBlockingWork(time) {
      const peek = this.peek();
      peek.executeBlockingProcess(time);
      this.manageTimeSlice(peek, time);
    }
