...
class Queue {
    ...
    manageTimeSlice(currentProcess, time) {
        if (!currentProcess.isStateChanged()) {
            ...
            if (this.quantumClock >= this.quantum) { // checks to see if this cycle will use up time
                this.quantumClock = 0;
                this.dequeue();
                if (!currentProcess.isFinished()) {
                    this.scheduler.handleInterrupt(this, SchedulerInterrupt.LOWER_PRIORITY);
                }
            }
        }
    }
    ...
    emitInterrupt(source, interrupt) {
        ...
        const proc = this.processes.splice(index, 1);
    }
}
