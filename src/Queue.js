...
class Queue {
    ...
    // [x] Manages a process's execution for the appropriate amount of time
    // [x] Checks to see if currentProcess's `this.stateChanged` property is true
    // [x] If it is, we don't want to give the process any time; reset `this.quantumClock` and return
    // [x] Otherwise, increment `this.quantumClock` by `time`
    // [x] Check to see if `this.quantumClock` is greater than `this.quantum`
    // [x] If it is, then we need to execute the next process in the queue
    // [x] Set `this.quantumClock` to 0
    // [x] Dequeue the next process from the queue
    // [x] If it isn't finished, emit a scheduler interrupt notifying the scheduler that this process
    // [x] needs to be moved to a lower priority queue
    manageTimeSlice(currentProcess, time) {
        ...
        if (this.quantumClock > this.quantum) {
            ...
            if (!proc.isFinished()) {
                this.scheduler.handleInterrupt(this, proc, SchedulerInterrupt.LOWER_PRIORITY);
            }
        }
    }
    ...
    emitInterrupt(source, interrupt) {
        const index = this.processes.findIndex((p, i) => source.pid === p.pid);
        if (index >= 0) {
            const proc = this.processes.splice(index, 1)[0];

            if (interrupt === SchedulerInterrupt.PROCESS_BLOCKED) this.scheduler.handleInterrupt(this, proc, SchedulerInterrupt.PROCESS_BLOCKED);
            if (interrupt === SchedulerInterrupt.PROCESS_READY) this.scheduler.handleInterrupt(this, proc, SchedulerInterrupt.PROCESS_READY);
        } else { console.log("ERR ERR ERR ERR ERR"); }
    }
}
