...
class Queue {
    ...
    manageTimeSlice(currentProcess, time) {
        if (currentProcess.isStateChanged()) {
            this.quantumClock = 0; // Is this a reset?
            return;
        } else {
    ...
    emitInterrupt(source, interrupt) {
        let index = this.processes.findIndex(source); // Find Process in Queue
        ...
        if (interrupt = SchedulerInterrupt.PROCESS_BLOCKED) {
            ...
            scheduler.handleInterrupt(this, extractedProcess[0], SchedulerInterrupt.PROCESS_BLOCKED)
