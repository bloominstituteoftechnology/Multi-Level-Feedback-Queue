...
class Queue {
    ...
    enqueue(process) {
        this.processes.push(process);   // push the process to the queue
        process.setParentQueue(this);   // set parent queue
        return process;                 // return the process that enqueued
    }
    ...
    manageTimeSlice(currentProcess, time) {
        if (currentProcess.isStateChanged()) {
            this.quantumClock = 0;          // if state changed, reset the queue's clock
            return;
        }
