...
class Scheduler { 
  ...
  emitInterrupt(queue, _process, interrupt) {
    switch (interrupt) {
    case 'PROCESS_BLOCKED':
      this.blockingQueue.enqueue(_process);
      break;
    case 'PROCESS_READY':
      this.addNewProcess(_process);
      break;
    case 'LOWER_PRIORITY':
      switch (queue.queueType) {
      case 'CPU_QUEUE':
        const lowerLevel = queue.priorityLevel === PRIORITY_LEVELS - 1 ? queue.priorityLevel : queue.priorityLevel + 1;
        this.runningQueues[lowerLevel].enqueue(_process);
        break;
      case 'BLOCKING_QUEUE':
        this.blockingQueue.enqueue(_process);
        break;
