...
class Scheduler {
  ...
  handleInterrupt(queue, process, interrupt) {
    ...
    if (interrupt === 'LOWER_PRIORITY') {
      const qType = QueueType;
      if (queue.getQueueType() === qType.BLOCKING_QUEUE) {
        this.blockingQueue.enqueue(process);
      }
      if (queue.getQueueType() === qType.CPU_QUEUE) {
        const prioLvl = queue.getPriorityLevel();
        if (prioLvl < PRIORITY_LEVELS - 1) {
          this.runningQueues[prioLvl + 1].enqueue(process);
        }
        this.runningQueues[prioLvl].enqueue(process);
      }
    }
  }
