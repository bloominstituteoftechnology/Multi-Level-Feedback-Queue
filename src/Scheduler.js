...
class Scheduler {
  ...
  // Initialize a variable with the current time and subtract current time by `this.clock`
  // to get the `workTime`
  ...
  // Then, iterate through all of the running queues and execute processes on those queues for
  // the amount of time given by `workTime`
  ...
  // In the case of a LOWER_PRIORITY interrupt, check to see if the input queue is a running queue
  // or blocking queue

  // If it is a running queue, add the process to the next lower priority queue, or back into
  // itself if it is already in the lowest priority queue
