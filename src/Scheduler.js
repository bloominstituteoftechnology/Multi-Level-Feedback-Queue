...
class Scheduler {
  ...
  run() {
    while (true) {
      let qTime = Date.now() - this.clock;
      this.clock = Date.now();

      const doBlocking = () => {
        this.blockingQueue.doBlockingWork(qTime);
        return;
      };
  ...
  allEmpty() {
    return this.runningQueues.filter((each) => {
      return !each.length.isEmpty();
    }).length === 0 && this.blockingQueue.isEmpty()
      ? false
      : true;
  }
