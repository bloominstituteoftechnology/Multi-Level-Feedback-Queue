...
class Scheduler {
    constructor() {
        ...
        for (let i = 0; i < PRIORITY_LEVELS; i++) {
            this.runningQueues[i] = new Queue(this, 10 + i * 20, i, QueueType.CPU_QUEUE); // create new queue at index i
        }
    }
    ...
    run() {
        // console.log("blockingqueue", this.blockingQueue);
        // console.log("runningqueues", this.runningQueues);
        while (!this.allEmpty()) {
            ...
            console.log("this.runningQueues.length", this.runningQueues.length);

            for (let i = 0; i < this.runningQueues.length; i++) {
                console.log("runningqueues[i].processes", this.runningQueues[i].processes);
                this.runningQueues[i].processes.executeProcess(workTime);
            }
        }
    }
    ...
    handleInterrupt(queue, process, interrupt) {
        // console.log("queue", queue, "process", process, "interrupt", interrupt);
        switch (interrupt) {
            ...
            case SchedulerInterrupt.LOWER_PRIORITY:
                switch (queue.getQueueType()) {
                    case QueueType.CPU_QUEUE: // runningblock
                        let level = queue.getPriorityLevel();
                        level++;
                        if (level > PRIORITY_LEVELS - 1) { // stop at priority level 3
