...
describe('Priority Boosing', () => {
    ...
    it('should run until all processes have completed execution', () => {
        const process1 = new Process(0, 100000);
        const process2 = new Process(1, 100000, true);
        const process3 = new Process(2, 100);
        ...
        const process3Found = (scheduler._boostedQueues().indexOf(process3) >= 0);
        ...
        expect(process1Found).toBe(true);
        expect(process2Found).toBe(true);
        expect(process3Found).toBe(false);
        expect(scheduler. _boostedQueues().length).toBe(2);
